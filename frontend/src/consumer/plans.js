...
export default function ConsumerPlans(props) {
  ...
  const BuyButton = ({ buttonText, onClick }) => (
    <button
      ...
      onMouseEnter={(e) => (e.target.style.background = '#02ffff', e.target.style.color = '#005d63')}
      onMouseLeave={(e) => (e.target.style.background = '#005d63', e.target.style.color = '#02ffff')}
    >
      {buttonText}
    </button>
  );
  ...
  async function purchaseEnergy(microGridId, value) {
    try {
      ...
      const totalAmount = bigValue.mul(multiplier);
      const producerAmount = totalAmount.div(10);

      await props.connect.purchaseEnergy_to_Producer(producers[0], { value: producerAmount });
      await props.connect.purchaseEnergy_to_Prosumer(prosumers[0], { value: totalAmount.sub(producerAmount) });

      const consumerData = await props.connect.address_Consumer(props.metaMaskAddress);
      sendDataToServer(consumerData[0], consumerData[1], value, totalAmount, microGridId);
    } catch (err) {
      console.error(err);
    }
  }
  ...
  return (
    <>
      <div style={{ backgroundColor: '#010c0e', minHeight: '100vh' }}>
        <ConsumerNavbar />
        <div className="container py-4">
          ...
          <div className="d-flex mb-4">
            ...
            <FontAwesomeIcon
              ...
              style={{ color: "#ffffff", padding: "0 15px", alignSelf: 'center' }}
            />
            <div style={{ marginLeft: 'auto' }}>
              <Link to="/carts">
                <FontAwesomeIcon icon={faCartShopping} style={{ color: "#ffffff", fontSize: '24px' }} />
              </Link>
            </div>
          </div>
              ...
              <div key={index} className="col-lg-4 col-md-6">
                ...
                >
                  <div className="card-body text-center">
                    <h5>{plan.units} Units</h5>
                    <p>Timespan: {plan.timespan}</p>
                    <p>Mobile: {plan.mobile_number}</p>
                    <p>Company: {plan.company_name}</p>
                    <div className="d-flex justify-content-around mt-3">
                      <BuyButton
                        buttonText="Buy Now"
                        onClick={() => purchaseEnergy(plan.microGridId, plan.units)}
                      />
